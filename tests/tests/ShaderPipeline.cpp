...
TEST(ShaderPipeline, DescriptorGroupReflection)
{
    ...
    GTEST_ASSERT_EQ(group3->descriptorCount(),2);
    ...
    GTEST_ASSERT_TRUE(group2->descriptor(0).shape().type == Descriptor::Type::STORAGE_BUFFER);
    GTEST_ASSERT_EQ(group2->descriptor(0).shape().arrayDepth,1);
    GTEST_ASSERT_EQ(group2->descriptor(0).shape().binding,1);
