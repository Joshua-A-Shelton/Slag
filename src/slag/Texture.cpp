...
namespace slag
{
    ...
    Texture* Texture::newTexture(const std::filesystem::path& imagePath, uint32_t mipLevels, TextureUsage usage, bool sRGB, Layout initializedLayout)
    {
        ...
        try
        {
            tex = lib::BackEndLib::get()->newTexture((void**) (&data), 1, w * h * Pixels::pixelBytes(Pixels::Format::R8G8B8A8_UINT),
                                                              sRGB ? Pixels::Format::R8G8B8A8_UNORM_SRGB : Pixels::Format::R8G8B8A8_UINT, Texture::Type::TEXTURE_2D, w, h, mipLevels, usage,
                                                              initializedLayout);
        }
