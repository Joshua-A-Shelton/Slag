...
TEST(Shader, UniformBufferDescriptionTypes)
{
    struct SimpleVertex
    {
        glm::vec3 position;
    ...
    frameBufferDescription.addColorTarget(Pixels::R8G8B8A8_UNORM);
    frameBufferDescription.setDepthTarget(Pixels::D32_FLOAT);
    std::unique_ptr<ShaderPipeline> basicShader = std::unique_ptr<ShaderPipeline>(ShaderPipeline::newShaderPipeline(modules.data(), modules.size(), nullptr, 0, properties, &vertexDescription, frameBufferDescription));

    auto description = basicShader->uniformBufferLayout(0,0);

    //need to test much more complicated shaders
    GTEST_FAIL();
}